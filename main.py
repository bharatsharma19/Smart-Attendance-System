...
def manually_fill():
    ...
    def fill_attendance():
        ...
        if subb == "":
            ...
            STUDENT_ENTRY = tk.Entry(
                MFW, width=20, bg="#dddddd", fg="red", font=("roboto", 23, " bold ")
            )
    ...
    SUB_ENTRY = tk.Entry(
        sb, width=20, bg="#F9F3E5", fg="red", font=("roboto", 20, " bold ")
    )
    ...
    if l1 == "":
        ...
        try:
            ...
            with open("StudentDetails\StudentDetails.csv", "a+") as csvFile:
                ...
                writer.writerow(row)
                csvFile.close()
            res = "Images Saved for Roll No. : " + Enrollment + " Name : " + Name
            Notification.configure(
                text=res, bg="SpringGreen3", width=50, font=("roboto", 18, "bold")
            )
...
def subjectchoose():
    def Fillattendances():
        sub = tx.get()
        now = time.time()  # To calculate seconds of video
        future = now + 20
        if time.time() < future:
            if sub == "":
                err_screen1()
    ...
    tx = tk.Entry(
        windo, width=20, bg="#F9F3E5", fg="red", font=("roboto", 20, " bold ")
    )
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == "bharat19":
            if password == "bharat19":
                win.destroy()
                import csv
                import tkinter
    ...
    un_entr = tk.Entry(
        win, width=20, bg="#F9F3E5", fg="red", font=("roboto", 23, " bold ")
    )
    ...
    pw_entr = tk.Entry(
        win, width=20, show="*", bg="#F9F3E5", fg="red", font=("roboto", 23, " bold ")
    )
...
message = tk.Label(
    ...
    width=50,
    height=3,
    font=("roboto, sans-serif", 34, "bold "),
)
...
txt2 = tk.Entry(window, width=20, bg="#F9F3E5",
                fg="red", font=("roboto", 25, " bold "))
